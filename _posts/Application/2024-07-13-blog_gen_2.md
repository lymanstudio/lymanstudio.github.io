...
블로그 포스트 캡처본은 특정 픽셀 높이 만큼 잘려져 있어 글이나 사진이 중간에 잘려 있는 경우가 많다. 따라서 한장씩 돌아가며 진행하다보면 이미지 분석이 안될 경우가 많다.
...
앞선 generate 함수에서 실제 LLM에 질의하는 get_response 함수를 작성해야 한다. 
...
스크립트를 참조해서 가장 비슷한 문장을 찾아 문장 안에 있는 텍스트를 포스트의 한줄처럼 출력하는 게 주요 임무야. 포스트와 똑같이 출력되게 HTML 형식으로 출력해줘.

참조할 정보는 '스크립트', '이전 소스 코드', '이전 소스코드에 사용된 스크립트 일부분' 세가지야.
...
- 완성된 소스코드는 사람이 알아보기 쉽게 prettify 해줘.
