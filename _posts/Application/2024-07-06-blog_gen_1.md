...
- `기 작성된 네이버 블로그 글을 가져온다.`
- 글, 그림 백업 후 포스트별로 정리
  
- 자동 네이버 포스트 캡처 기능
- `작성된 네이버 글을 참조하여 가져온 글, 그림을 이용해 포맷팅 된 상태로 구성한다`

  - 캡처된 포스트와 글을 참조해 plain text를 포멧팅된 형태로 바꿔주는 생성 모델 => 모델 A
- `새로운 글에 대해 자동으로 포스트 글로 바뀌도록 모델을 구성한다.`
...
- 여러 개의 글을 한번에 저장 가능하며 각 포스트 마다 저장 용량이 나온다. 한번에 500MB까지 저장가능하다. 만들기를 눌러준다.
  <img src="./../../images/2024-07-06-blog_gen_1/image-20240706105358155.png" alt="image-20240706105358155" style="zoom:80%;" />
- 그러면 생성중이라고 하며 파일이 생성되고 마친 뒤 다운로드를 눌러 받아주면 된다.
...
그리고 이 때 마지막으로 포스트 자동 스크린 캡처(`screen_capture_loop(post_dir, prv_post_id))`도 같이 진행한다. 



# Step2.  네이버 블로그 포스트 자동 캡쳐 기능
